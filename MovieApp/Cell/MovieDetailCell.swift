...
class MovieDetailCell: UICollectionViewCell {
    ...
    private func configUI() {
        ...
        [movieImage,
         movieName,
         generalView].forEach { contentView.addSubview($0) }
    }
    ...
    private func configConstraints() {
        NSLayoutConstraint.activate([
            ...
            movieImage.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: 32),
            movieImage.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -32),
            ...
            generalView.leadingAnchor.constraint(equalTo: contentView.leadingAnchor, constant: 32),
            generalView.trailingAnchor.constraint(equalTo: contentView.trailingAnchor, constant: -32),
    ...
    func config(movie: MovieResult) {
        guard let imageURL = URL(string: "\(NetworkHelper.imageURL)/\(movie.posterPath ?? "")") else { return }
        movieImage.kf.setImage(with: imageURL,
                               placeholder: UIImage(named: "placeholder"))
        movieName.text = movie.title
        movieCountry.text = movie.originalLanguage?.capitalized
        self.movieHour.text = ""
        movieRating.text = String(String(movie.voteAverage ?? 0).prefix(1))
        self.movie = movie
    }
}
